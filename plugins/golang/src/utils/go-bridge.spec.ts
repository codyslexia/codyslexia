...
jest.mock('child_process', () => ({
  execSync: jest.fn().mockReturnValue(Buffer.from('go version go1.21.1 windows/amd64\n', 'utf-8')),
}))
...
describe('Go bridge', () => {
  ...
  describe('Method: getGoVersion', () => {
    ...
    it('should throw error if go version not found', () => {
      ...
      expect(() => getGoVersion()).toThrow('Cannot retrieve current Go version')
    })
  })
  ...
  describe('Method: getGoShortVersion', () => {
    ...
    it('should throw error if go version not found', () => {
      jest.spyOn(child_process, 'execSync').mockImplementationOnce(() => null)
      expect(() => getGoShortVersion()).toThrow('Cannot retrieve current Go version')
    })
  })
  ...
  describe('Method: supportsGoWorkspace', () => {
    it.each`
      ...
      ${'1.20'}   | ${true}
      ${'2.10'}   | ${true}
    `('should return $expected if version is $version', ({ version, expected }) => {
      jest
        .spyOn(child_process, 'execSync')
        .mockReturnValueOnce(Buffer.from(`go version go${version} windows/amd64`, 'utf-8'))
  ...
  describe('Method: addGoWorkDependency', () => {
    ...
    it.each`
      ...
      ${'go 1.21\n\nuse ./app1\n'}                   | ${'go 1.21\n\nuse (\n\t./app1\n\t./new-app\n)\n'}
      ${'go 1.21\n\nuse (\n\t./app1\n\t./app2\n)\n'} | ${'go 1.21\n\nuse (\n\t./app1\n\t./app2\n\t./new-app\n)\n'}
    `('should add new dependency to go.work with content $content', ({ content, newContent }) => {
