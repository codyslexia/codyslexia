...
import { GO_MOD_FILE, GO_WORK_FILE, GO_WORK_MINIMUM_VERSION } from '../constants'
 ...
 */
export const parseGoList = (listType: GoListType, content: string): string[] => {
 ...
 */
export const createGoMod = (tree: Tree, name: string, folder?: string): void => {
...
export const addGoWorkDependency = (tree: Tree, projectRoot: string): void => {
  ...
  const modules = [...new Set([...exisitingModules, `./${projectRoot}`])].sort((m1, m2) =>
    m1.localeCompare(m2)
  )
