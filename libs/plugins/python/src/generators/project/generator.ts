...
function addPackageSource(normalizedOptions: NormalizedSchema, host: Tree) {
  if (normalizedOptions.customSource) {
    ...
    const pyprojectTomlPath = path.join(normalizedOptions.projectRoot, 'pyproject.toml')
...
async function generator(host: Tree, options: Schema) {
  ...
  addProjectConfiguration(host, normalizedOptions.projectName, {
    ...
    targets: {
      ...
      lint: {
        executor: '@plugins/python:flake8',
        outputs: [`{workspaceRoot}/reports/${normalizedOptions.projectRoot}/pylint.txt`],
        options: {
