...
type IntegerPart<T extends string> = T extends `${infer I}.${infer T}` ? I : T
...
export type RGB<T extends string> = T extends `rgb(${infer R},${infer G},${infer B})`
...
export type RGBA<T extends string> = T extends `rgba(${infer R},${infer G},${infer B},${infer O})`
...
export type HSL<T extends string> = T extends `hsl(${infer H},${infer S},${infer L})`
  ? `111` extends `${Degree<Trim<H>>}${IsPercent<Trim<S>>}${IsPercent<Trim<L>>}`
    ? T
    : never
  : never

export type HSLA<T extends string> = T extends `hsla(${infer H},${infer S},${infer L},${infer O})`
