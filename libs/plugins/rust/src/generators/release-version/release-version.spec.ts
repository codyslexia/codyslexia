...
To fix this you will either need to add a Cargo.toml file at that location, or configure "release" within your nx.json to exclude "my-lib" from the current release group, or amend the packageRoot configuration to point to where the Cargo.toml should be.`,
  ...
  describe('fixed release group', () => {
    it(`should work with semver keywords and exact semver versions`, async () => {
      expect(parseCargoTomlWithTree(tree, 'libs/my-lib', 'my-lib').package.version).toEqual('0.0.1')
      await releaseVersionGenerator(tree, {
        projects: Object.values(projectGraph.nodes), // version all projects
        projectGraph,
        specifier: 'major',
      ...
      expect(parseCargoTomlWithTree(tree, 'libs/my-lib', 'my-lib').package.version).toEqual('1.0.0')
      ...
      expect(parseCargoTomlWithTree(tree, 'libs/my-lib', 'my-lib').package.version).toEqual('1.1.0')
      ...
      expect(parseCargoTomlWithTree(tree, 'libs/my-lib', 'my-lib').package.version).toEqual('1.1.1')
      ...
      expect(parseCargoTomlWithTree(tree, 'libs/my-lib', 'my-lib').package.version).toEqual('1.2.3')
    })
