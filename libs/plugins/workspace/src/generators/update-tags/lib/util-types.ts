 ...
 * const pet: Pet = '' // no hints on IDE
 * type Pet2 = LiteralUnion<'dog' | 'cat', string>
 * const pet: Pet2 = '' // all good
 * ```
 * @category Type
 */
...
declare namespace PackageJson {
  ...
  export interface WorkspaceConfig {
    ...
    /**
     * By adding workspace pattern strings here, Yarn will resume non-workspace behavior for any package
     * which matches the defined patterns. [Read more](https://classic.yarnpkg.com/blog/2018/02/15/nohoist/)
     */
  ...
  export interface YarnConfiguration {
     ...
     * Workspaces allow you to manage multiple packages within the same repository in such a way
     *
     * Note that you only need to run `yarn install` once to install all of them in a single pass.
     * Please note that the top-level `private` property of `package.json` **must** be set to `true` in order to use workspaces.
     */
    workspaces?: WorkspacePattern[] | WorkspaceConfig
    /**
     * If your package only allows one version of a given dependency, and you’d like to enforce
     * the same behavior as `yarn install --flat` on the command-line, set this to `true`.
     *
     * Note that if your `package.json` contains `"flat": true` and other packages depend on yours
     * (e.g. you are building a library rather than an app), those other packages will also need
     * `"flat": true` in their `package.json` or be installed with `yarn install --flat` on the command-line.
     */
    flat?: boolean
    /**
     * Selective version resolutions. Allows the definition of custom package versions
     * inside dependencies without manual edits in the `yarn.lock` file.
     */
  ...
  /**
   * Type for [npm's `package.json` file](https://docs.npmjs.com/creating-a-package-json-file) with standard npm properties.
   */
  export interface PackageJsonStandard {
    ...
    main?: string
    /** Subpath exports to define entry points of the package. [Read more.](https://nodejs.org/api/packages.html#subpath-exports) */
    exports?: Exports
    /**
     * Subpath imports to define internal package import maps that only apply to import specifiers
     * from within the package itself. [Read more.](https://nodejs.org/api/packages.html#subpath-imports)
     */
    imports?: Imports
     ...
     * Useful if the package is primarily a command-line application that should be installed globally.
     * @deprecated
     */
  ...
  export interface PublishConfig {
     ...
     * The only valid values for access are public and restricted. Unscoped packages always have an access level of public.
     */
