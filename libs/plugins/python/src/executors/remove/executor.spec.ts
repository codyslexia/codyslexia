...
describe('Delete Executor', () => {
  ...
  beforeEach(() => {
    ...
    getPoetryVersionMock = jest.spyOn(poetryUtils, 'getPoetryVersion').mockResolvedValue('1.5.0')
    activateVenvMock = jest.spyOn(poetryUtils, 'activateVenv').mockReturnValue(undefined)
...
`,
    ...
    expect(spawnSyncMock).toHaveBeenNthCalledWith(1, 'poetry', ['remove', 'click', '-vvv'], {
      cwd: 'apps/app',
      shell: false,
      stdio: 'inherit',
    })
...
`,
    ...
    expect(activateVenvMock).toHaveBeenCalledWith('.')
    expect(spawnSyncMock).toHaveBeenCalledTimes(1)
    expect(spawnSyncMock).toHaveBeenNthCalledWith(1, 'poetry', ['remove', 'click'], {
      cwd: 'apps/app',
      shell: false,
      stdio: 'inherit',
    })
  ...
  it('run remove target and should remove the dependency to the project using --lock when the root pyproject.toml is present (poetry version 1.5.0)', async () => {
    ...
    expect(checkPoetryExecutableMock).toHaveBeenCalled()
    expect(activateVenvMock).toHaveBeenCalledWith('.')
    expect(spawnSyncMock).toHaveBeenNthCalledWith(1, 'poetry', ['remove', 'click', '--lock'], {
      cwd: 'apps/app',
      shell: false,
      stdio: 'inherit',
    })
    expect(spawnSyncMock).toHaveBeenNthCalledWith(2, 'poetry', ['lock', '--no-update'], {
      shell: false,
      stdio: 'inherit',
    })
