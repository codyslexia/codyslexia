...
describe('Either', () => {
  ...
  it('should create a left instance', () => {
    const either = E.left(new UnprocessableEntity())
    expect(either.isRight()).toBeFalsy()
    expect(either.isLeft()).toBeTruthy()

    if (either.isLeft()) {
      expect(either.left).toBeInstanceOf(UnprocessableEntity)
    }
  })
  ...
  it('should map a left instance', () => {
    ...
    if (mapped.isLeft()) {
      expect(mapped.left).toBeInstanceOf(UnprocessableEntity)
    }
  })
  ...
  it('should chain a left instance', () => {
    ...
    if (chained.isLeft()) {
      expect(chained.left).toBeInstanceOf(UnprocessableEntity)
    }
  })

  it('should chain an error', () => {
    const either = E.left(new UnprocessableEntity())
    const chained = E.chainError(either, (error) => E.left(new Error(error.message)))
    expect(chained.isRight()).toBeFalsy()
    expect(chained.isLeft()).toBeTruthy()
