...
import { Stateful, Computed } from './types'
...
describe('State', () => {
  ...
  test('it should set new state (set method)', () => {
    const initialState = createState<Planet>({
      name: 'Earth',
      system: 'Solar system',
      moons: [{ name: 'Moon' }],
    })
    ...
    initialState.set(newState)

    expect(initialState.get()).toMatchObject(newState)
  })
  ...
  test('it should update element (update)', () => {
    ...
    planetState.update(({ moons }) => {
      moons?.push({ name: 'Deimos' })
    })
  ...
  test('it should react on changes (subscripe method)', () => {
    ...
    expect(isTheMoonGettingCloser).toBe(false)
  ...
  test('it should return count of subscribers (getActiveSubscribersCount method)', () => {
    ...
    const APOGEE = 405400
    const moon = createState<MoonObserver>({
      distance: APOGEE,
    })
    ...
    const unsubscribeMoon = moon.subscribe(({ distance }) => {
      isTheMoonGettingCloser = distance > APOGEE
    })

    moon.update(({ distance }) => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      distance -= 30
    })

    expect(moon.getActiveSubscribersCount()).toBe(2)

    unsubscribeMoon()

    expect(moon.getActiveSubscribersCount()).toBe(1)
  })

  test('it should use custom condition for update (condition)', () => {
    ...
    moonObserverState.subscribe(
      ...
      { condition: (s) => s.distance === perigee }
    )
  ...
  test('it should delete broken subscriber', () => {
    ...
    moonObserverState.subscribe(() => {
      throw new Error('Please, fix me. I am broken.')
    }, {})
  ...
  test('it should delete all unprotected subscribers (unsubscribeAll method)', () => {
    ...
    moonObserverState.subscribe(
      ...
      { condition: (s) => s.distance === perigee, protected: true }
    )
    ...
    moonObserverState.subscribe(
      ...
      { condition: (s) => s.distance === APOGEE, protected: false }
    )
  ...
  test('it should async set new state', async () => {
    ...
    const asyncAction = async (stateManager: Stateful<Planet | null>) => {
  ...
  test('it should update selector', () => {
    ...
    const planetName: Computed<string> = planetState.createSelector(planetNameSelector)
...
describe('State Security', () => {
  ...
  test("it should throw the error for 'fill' method outside the 'update' method", () => {
    ...
    const expectErr = () => planetState.get().moons?.fill({ name: 'Moon' }, 2, 4)
  ...
  test("it should throw the error for 'push' method outside the 'update' method", () => {
    ...
    const expectErr = () => planetState.get().moons?.push({ name: 'Deimos' })
  ...
  test("it should throw the error for 'unshift' method outside the 'update' method", () => {
    ...
    const expectErr = () => planetState.get().moons?.unshift({ name: 'Deimos' })
...
describe('State Computed', () => {
  test('it should create computed state and react on changes (subscripe method)', () => {
    ...
    const remaningTravelTimeComputedState = createComputedState<string>((): string => {
      const speedKmPerSec = speedState.get() / 3600 // convert km/h to km/s
      const remainingDistance = DISTANCE - speedKmPerSec * travelTimeState.get() // calculate remaining distance
  ...
  test('it should throw an error due to a missing dependency states array', () => {
    ...
    const expectErr = () =>
      createComputedState<string>((): string => {
        return problemState.get() ? 'Houston, we have a problem' : 'Houston, everything is fine'
      }, [])
  ...
  test("It shouldn't work to change directly", () => {
    ...
    expect(statusComputedState.get()).toEqual('Houston, everything is fine')
  })
})
...
describe('State Utils', () => {
  test('It should format error', () => {
    ...
    expect(formatError(description, new Error(errorMsg))).toEqual(`${description}: ${errorMsg}`)
  })
})

describe('State Transactions', () => {
  it('should add a transaction', () => {
    const transactionService = new StateTransactions<number>(9)
  ...
  it('should get all transactions', () => {
    const transactionService = new StateTransactions<number>(1)
  ...
  it('should get a transaction by number', () => {
    const transactionService = new StateTransactions<number>(3)
