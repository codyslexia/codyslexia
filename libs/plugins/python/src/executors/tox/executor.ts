...
export default async function executor(options: ToxExecutorSchema, context: ExecutorContext) {
  ...
  try {
    ...
    const packageFile = readdirSync(distFolder).find((file) => file.endsWith('.tar.gz'))
