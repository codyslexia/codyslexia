...
describe('library generator', () => {
  ...
  it('should write project configuration', async () => {
    ...
    expect(nxDevkit.addProjectConfiguration).toHaveBeenCalledWith(tree, 'data-access', {
  ...
  it('should create Go mod for project if in a Go workspace', async () => {
    ...
    await libraryGenerator(tree, options)
    expect(shared.createGoMod).toHaveBeenCalledWith(tree, 'proj/dataaccess', 'libs/data-access')
  })
  ...
  it('should add Go work dependency if in a Go workspace', async () => {
    jest.spyOn(shared, 'isGoWorkspace').mockReturnValueOnce(true)
    await libraryGenerator(tree, options)
    expect(shared.addGoWorkDependency).toHaveBeenCalledWith(tree, 'libs/data-access')
  })

  it('should add tidy executor for project if in a Go workspace', async () => {
    jest.spyOn(shared, 'isGoWorkspace').mockReturnValueOnce(true)
    await libraryGenerator(tree, options)
    expect(nxDevkit.updateProjectConfiguration).toHaveBeenCalledWith(tree, 'data-access', {
      root: 'libs/data-access',
      projectType: 'library',
      sourceRoot: 'libs/data-access',
      targets: {
