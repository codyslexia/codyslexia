...
export function map<E extends Error, T, U>(either: Either<E, T>, f: (value: T) => U): Either<E, U> {
  if (either.isRight()) {
...
export function getOrElse<E extends Error, T>(either: Either<E, T>, defaultValue: T): T {
  if (either.isRight()) {
    return either.right
  } else {
    return defaultValue
