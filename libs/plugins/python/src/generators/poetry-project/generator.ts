...
import { PyprojectToml, PyprojectTomlDependencies } from '../../graph/dependency-graph'
...
function normalizeOptions(tree: Tree, options: PoetryProjectGeneratorSchema): NormalizedSchema {
  const { projectName, projectRoot } = calculateProjectNameAndRoot(options, tree)
  ...
  if (options.devDependenciesProject) {
    const projectConfig = readProjectConfiguration(tree, options.devDependenciesProject)
    newOptions.devDependenciesProjectPath = path.relative(projectRoot, projectConfig.root)
  }
...
function calculateProjectNameAndRoot(options: PoetryProjectGeneratorSchema, tree: Tree) {
...
function getPyTestAddopts(
  ...
  if (options.unitTestRunner === 'pytest') {
    ...
    if (options.codeCoverage && options.codeCoverageXmlReport) {
      args.push(`--cov-report xml:'${offset}coverage/${projectRoot}/coverage.xml'`)
    }

    if (options.unitTestHtmlReport) {
      args.push(`--html='${offset}reports/${projectRoot}/unittests/html/index.html'`)
    }

    if (options.unitTestJUnitReport) {
      args.push(`--junitxml='${offset}reports/${projectRoot}/unittests/junit.xml'`)
    }
...
function addFiles(tree: Tree, options: NormalizedSchema) {
  ...
  if (options.templateDir) {
    generateFiles(tree, path.join(options.templateDir), options.projectRoot, templateOptions)
    return
  }

  generateFiles(tree, path.join(__dirname, 'files', 'base'), options.projectRoot, templateOptions)
...
function updateDevDependenciesProject(host: Tree, normalizedOptions: NormalizedSchema) {
  if (normalizedOptions.devDependenciesProject) {
...
function getPyprojectTomlByProjectName(host: Tree, projectName: string) {
  ...
  const pyprojectToml = parse(host.read(pyprojectTomlPath, 'utf-8')) as PyprojectToml
...
function addTestDependencies(
  ...
  if (normalizedOptions.unitTestRunner === 'pytest' && !dependencies['pytest']) {
    dependencies['pytest'] = '7.3.1'
  }
  if (normalizedOptions.unitTestRunner === 'pytest' && !dependencies['pytest-sugar']) {
