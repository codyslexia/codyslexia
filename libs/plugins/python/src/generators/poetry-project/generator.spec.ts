...
describe('application generator', () => {
  ...
  describe('as-provided', () => {
    it('should run successfully minimal configuration', async () => {
      ...
      expect(appTree.exists(`${projectDirectory}/tests/test_hello.py`)).toBeFalsy()
    })
  ...
  describe('individual package', () => {
    ...
    it('should run successfully with linting and testing options with an existing dev dependency project', async () => {
      ...
      appTree.write('libs/shared/dev-lib/pyproject.toml', stringify(pyprojectToml))
  ...
  describe('shared virtual environment', () => {
    it('should run successfully with minimal options', async () => {
      ...
      expect(spawnSyncMock).toHaveBeenNthCalledWith(1, 'poetry', ['lock', '--no-update'], {
        shell: false,
        stdio: 'inherit',
      })
  ...
  describe('custom template dir', () => {
    it('should run successfully with custom template dir', async () => {
      ...
      expect(appTree.read('apps/test/pyproject.toml', 'utf-8')).toMatchSnapshot()
...
function assertGeneratedFilesBase(appTree: Tree, projectDirectory: string, moduleName: string) {
  expect(appTree.exists(`${projectDirectory}/README.md`)).toBeTruthy()
  expect(appTree.read(`${projectDirectory}/README.md`, 'utf8')).toMatchSnapshot()

  expect(appTree.exists(`${projectDirectory}/pyproject.toml`)).toBeTruthy()
  expect(appTree.read(`${projectDirectory}/pyproject.toml`, 'utf8')).toMatchSnapshot()

  expect(appTree.exists(`${projectDirectory}/${moduleName}/hello.py`)).toBeTruthy()

  expect(appTree.read(`${projectDirectory}/${moduleName}/hello.py`, 'utf-8')).toMatchSnapshot()
...
function assertGeneratedFilesFlake8(appTree: Tree, projectDirectory: string) {
  ...
  expect(appTree.read(`${projectDirectory}/.flake8`, 'utf-8')).toMatchSnapshot()
}

function assertGeneratedFilesPyTest(appTree: Tree, projectDirectory: string) {
  expect(appTree.exists(`${projectDirectory}/tests/test_hello.py`)).toBeTruthy()

  expect(appTree.read(`${projectDirectory}/tests/test_hello.py`, 'utf-8')).toMatchSnapshot()
}
